...
try{
   ...
   error_log_pr('rasa',"                {$utterance} ***Api Called");
    ...
    try{
       ...
       // $pagiClient->consoleLog('***************before save talk************************* 123');
        $data=array('event'=>$pagiClient->getVariable('event'),'questionid'=>$pagiClient->getVariable('questionid'),'data'=>$utterance,'msg'=>$result['msg']);
   
        $pagiClient->consoleLog('***************'.$data['event'].'************************* ');
       // $pagiClient->consoleLog('***************'.$data->event.'************************* ');
       $pagiClient->consoleLog($call->id);
       //,$repeatquestion,false,$result
        $firstInterView->save_talk($data,$call);
    $pagiClient->setVariable('event','continue');
    
    $pagiClient->consoleLog('***************after  save talk************************* ');
   // $pagiClient->consoleLog('Message: ' .$e->getMessage());
    }catch(Exception $e){
        $pagiClient->consoleLog('*****************exception save talk*********************** ');
        $pagiClient->consoleLog('Message: ' .$e->getMessage());
    }

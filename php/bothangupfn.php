...
function getfinalresponse($id){
    $urlsync='http://192.168.31.203:5010/'.$id;
     $curl = curl_init();
    ...
    return $response;
         ...
         $result=getfinalresponse(6412);
         var_dump($result['interest']);
         exit();
...
function callapi($sender,$message){
    ...
    curl_setopt_array($curl, array(
      ...
      CURLOPT_URL => "http://192.168.31.203:5002/webhooks/rest/webhook",
        ...
        //$this->pagiClient->consoleLog('current question  ********************'.$speech.'**************');
          ...
          //  $firstInterView=new FirstInterviewHelper();
           ...
           $pagiClient->setVariable('dddddddd',json_encode($result));
            ...
            //$result=callapi($callid,'');
           //$call->disposition=$result['interest'];            //
